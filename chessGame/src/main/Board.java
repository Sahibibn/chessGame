...
public class Board extends JPanel {
    ...
    public Piece getPiece(int col, int row) {  // Fixed parameter order
        for (Piece piece : pieceList) {
            if (piece.col == col && piece.row == row) {  // Fixed check order
                return piece;
            }
        }
        return null;
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                g2d.setColor((c + r) % 2 == 0 ? new Color(38, 38, 41) : new Color(243, 237, 230, 255));
        ...
        if (selectedPiece != null) {
            for (int r = 0; r < rows; r++) {
                for (int c = 0; c < cols; c++) {
                    ...
                    if (isValidMove(move)) {
                        g2d.setColor(new Color(68, 180, 57, 150)); // Semi-transparent highlight
                        g2d.fillRect(c * tileSize, r * tileSize, tileSize, tileSize);
                    }
                }
            }
